...
export default function LeaderBoardSection() {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute -bottom-[38rem] right-[14%] w-[950px] h-[1000px]">
        <Image
          src="/leader_board_one.png"
          alt="Leaderboard"
          width={950}
          height={1000}
          ...
          {!loading && !error && remainingUsers.length > 0 && (
            <ul className="space-y-4 w-[108%] mt-[1.5rem] ml-[1rem]">
              {remainingUsers.map((user, index) => (
                <li
                  key={user.id}
                  className="flex items-center bg-[#f4c60c9a] p-4 border border-yellow-500 rounded-lg shadow-lg text-white w-[90%] h-[5rem] gap-2"
                >
